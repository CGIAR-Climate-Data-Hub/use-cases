...
- **Open items (handover):**
  - **Theme 3 hazard-exposure S3 mismatch** — the CR notebook reads `nex-gddp-cmip6/.../vop_nominal-usd21/ENSEMBLEmean`, the pipeline writes `atlas_cmip6/.../vop_intld15/ENSEMBLE` (path + currency differ). Flagged NOT-YET-S3-VERIFIED; confirm with `aws s3 ls` before trusting the "Present" claim.
